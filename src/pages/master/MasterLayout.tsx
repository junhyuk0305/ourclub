...
export function MasterLayout() {
  ...
  return (
    <div className="flex flex-col h-screen bg-sand-50 overflow-hidden font-sans">
      {/* 헤더 */}
      <header className="h-14 border-b border-sand-200 bg-ink text-white flex items-center px-6 gap-4 shrink-0">
        <Shield className="w-5 h-5 text-brand-accent" strokeWidth={2.5} />
        <span className="font-black text-lg tracking-tight">OURCLUB 마스터</span>
        <span className="ml-auto text-sm font-bold text-sand-400">{profile?.name}</span>
        <button
          onClick={signOut}
          className="flex items-center gap-1 text-sm font-bold text-sand-400 hover:text-brand-accent transition-colors"
        >
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* 사이드바 */}
        <aside className="w-56 border-r border-sand-200 bg-white flex flex-col p-4 gap-1 shrink-0">
          {NAV.map(({ path, label, icon: Icon }) => {
            ...
            return (
              <Link
                key={path}
                to={path}
                className={`flex items-center gap-3 px-3 py-3 font-bold text-sm rounded-ctl transition-colors ${
                  active
                    ? 'bg-ink text-white font-black'
