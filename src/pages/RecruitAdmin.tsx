...
import { Search, CheckSquare, Mail, UserCheck, X, Check, Edit2, Loader } from 'lucide-react';
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 bg-gray-50 flex flex-col overflow-y-auto p-8">
          ...
          <div className="bg-white border border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex-1 overflow-auto">
            {fetching ? (
              <div className="flex items-center justify-center py-20"><Loader className="w-8 h-8 animate-spin text-orange-500" /></div>
            ) : (
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-gray-200">
                  {filtered.length === 0 ? (
                    <tr><td colSpan={7} className="p-10 text-center text-gray-500 font-bold">지원자가 없습니다.</td></tr>
                  ) : filtered.map(app => (
                    <tr key={app.id} className={`hover:bg-orange-50 transition-colors ${selectedIds.includes(app.id) ? 'bg-orange-50' : ''}`}>
                      <td className="p-4"><input type="checkbox" checked={selectedIds.includes(app.id)} onChange={() => toggle(app.id)} className="w-4 h-4 accent-orange-500" /></td>
                      ...
                      <td className="p-4 text-center flex justify-center gap-2">
                        ...
                        {app.status !== '최종합격' && (
                          <button onClick={() => { setSelectedApplicant(app); setEmailBody(`안녕하세요, ${app.profiles?.name ?? ''}님!\n최종 합격을 진심으로 축하드립니다.`); setShowEmailModal(true); }}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-end bg-black/50" onClick={onClose}>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-8 flex flex-col gap-6">
          ...
          {/* 운영진 평가 */}
          <div className="border-2 border-black p-6 bg-gray-50 flex gap-4">
            <div className="flex-1">
              <label className="font-black text-sm block mb-1">서류 점수 (0~100)</label>
              <input type="number" value={score} onChange={e => setScore(e.target.value)} min={0} max={100}
                className="w-full p-2 border border-black font-bold outline-none focus:border-orange-500" placeholder="85" />
            </div>
            <div className="flex-[3]">
              <label className="font-black text-sm block mb-1">운영진 메모</label>
              <textarea value={note} onChange={e => setNote(e.target.value)} rows={2}
                className="w-full p-2 border border-black font-bold outline-none focus:border-orange-500 resize-none" placeholder="내부 검토 메모" />
            </div>
            <div className="flex items-end">
              <button onClick={() => onSaveNote(applicant.id, score ? parseInt(score, 10) : null, note)}
                className="px-4 py-2 bg-black text-white font-bold hover:bg-orange-500 hover:text-black transition-colors">
                저장
              </button>
            </div>
          </div>
          ...
          {Object.entries(applicant.answers).map(([key, val]) => (
            <div key={key} className="border border-black p-6 bg-orange-50">
              ...
              <p className="font-medium text-gray-800 leading-relaxed">{val}</p>
            </div>
          ))}
          ...
          <div className="flex gap-2">
            {(['서류심사', '면접', '불합격'] as AppStatus[]).map(s => (
              <button key={s} onClick={() => onStatusChange(applicant.id, s)}
                className={`px-4 py-2 border border-black font-bold ${applicant.status === s ? 'bg-black text-white' : 'bg-white hover:bg-gray-100'}`}>
                {s}
              </button>
            ))}
          </div>
          <button onClick={onFinalAccept}
            className="px-8 py-3 bg-orange-500 border-2 border-black font-black text-white hover:bg-orange-600 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-none hover:translate-y-1 transition-all flex items-center gap-2">
